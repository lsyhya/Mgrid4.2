...
public class SgTriggerSetter extends TextView implements IObject {
	public SgTriggerSetter(Context context) {
		...
		/*
		 * try { m_oEditText.setBackgroundDrawable(getImageDrawable(
		 * "/mnt/sdcard/YADA/edittext.png")); } catch (IOException e) { // TODO
		...
		// TODO: 样式临时写死
		m_oEditText
				.setFilters(new InputFilter[] { new InputFilter.LengthFilter(7) }); // TODO:
																					// 写死可输入位数
		m_oEditText.setTextColor(Color.BLACK);
		// m_oEditText.setBackgroundDrawable(new
		// BitmapDrawable(Environment.getExternalStorageDirectory().getPath() +
		// "/MGridRes/edittext.png"));
	...
	public void doLayout(boolean bool, int l, int t, int r, int b) {
		...
		if (m_rRenderWindow.isLayoutVisible(m_rBBox)) {
			...
			m_oEditText.layout(nX, nY + nHeight / 10, nX
		...
		if ("".equals(m_strCmdExpression) == false) {
			synchronized (m_rRenderWindow.m_oShareObject) {
				m_rRenderWindow.m_oShareObject.m_mapTriggerCommand.put(
						getUniqueID(), m_oEditText.getText().toString().trim());
			}
		}
	 ...
	 * @return
	 * @throws IOException
	 *             \/ private static final int BUFFER_SIZE = 1024 * 16; // 最大 16
	 *             K public BitmapDrawable getImageDrawable(String path) throws
	 *             IOException { //打开文件 File file = new File(path);
	 *             if(!file.exists()) { return null; }
	 * 
	 *             ByteArrayOutputStream outStream = new
	 *             ByteArrayOutputStream(); byte[] bt = new byte[BUFFER_SIZE];
	 * 
	 ...
	 *             in.close();
	 * 
	 *             //转换成byte 后 再格式化成位图 byte[] data = outStream.toByteArray();
	 *             Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0,
	 *             data.length);// 生成位图 BitmapDrawable bd = new
