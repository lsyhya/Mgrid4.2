...
/** 布局列表 */
public class SgTable extends View implements IObject, ViewObjectSetCallBack {

	public ViewObjectBase base = new TableObject();

	public SgTable(Context context) {
		super(context);

		/*
		 * //setBackgroundColor(0x00FFFFFF); this.setOnTouchListener(new
		 * OnTouchListener() {
		 * 
		 * @Override public boolean onTouch(View view, MotionEvent event) { return
		 * false; } });
		 */
	...
	protected void onDraw(Canvas canvas) {
		...
		// int nX = m_rRenderWindow.getLeft() + (int) (((float)m_nPosX /
		// (float)MainWindow.FORM_WIDTH) * (m_rRenderWindow.VIEW_RIGHT -
		// m_rRenderWindow.VIEW_LEFT));
		// int nY = m_rRenderWindow.getTop() + (int) (((float)m_nPosY /
		...
		for (int i = 0; i < m_nColNum - 1; ++i)
			canvas.drawRect(nX + fFirstCol + i * fGridWidth, nY, nX + fFirstCol + (i + 1) * fGridWidth, nY + fFirstRow,
					m_oPaint);
		// 第一列
		for (int i = 0; i < m_nRowNum - 1; ++i)
			canvas.drawRect(nX, nY + fFirstRow + i * fGridHeight, nX + fFirstCol,
					nY + fFirstRow + (i + 1) * fGridHeight, m_oPaint);
		// 右下表格
		for (int i = 0; i < m_nRowNum - 1; i++) {
			for (int j = 0; j < m_nColNum - 1; j++) {
				canvas.drawRect(nX + fFirstCol + j * fGridWidth, nY + fFirstRow + i * fGridHeight,
						nX + fFirstCol + (j + 1) * fGridWidth, nY + fFirstRow + (i + 1) * fGridHeight, m_oPaint);
			}
		}
	}
	...
	public void doLayout(boolean bool, int l, int t, int r, int b) {
		...
		if (m_rRenderWindow.isLayoutVisible(m_rBBox)) {
			...
			// 表头 | 首行
			if (m_bIsHasHead)
				m_vTableTitleClo.layout(nX, nY + fFirstRow, nX + fFirstCol,
						nY + fFirstRow + (m_nRowNum - 1) * fGridHeight);

			// 首列
			m_vTableTitleRow.layout(nX + fFirstCol, nY, nX + fFirstCol + (m_nColNum - 1) * fGridWidth, nY + fFirstRow);

			// 表格
			layout(nX - m_nLineThickness, nY - m_nLineThickness, nX + nWidth + m_nLineThickness,
					nY + nHeight + m_nLineThickness);
		}
		// */
	}
	...
	private String parseColor(String strValue) {
		...
		if ("Highlight".equalsIgnoreCase(strValue) || "MenuHighlight".equalsIgnoreCase(strValue)) {
			...
			if(strValue.contains("#"))
			{
				...
				{
					return "#"+strValue.substring(3, strValue.length());
				}
	...
	public void onCall() {
		...
		((TableObject) base).setColNum(m_nColNum);
		((TableObject) base).setRowNUm(m_nRowNum);
