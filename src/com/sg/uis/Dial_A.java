...
/**自定义表盘 made fjw 2016 04 23 */
public class Dial_A extends View implements IObject {
	public Dial_A(Context context) {  
        super(context); 
		...
		RectF rect = new RectF(m_rRectF1.left, m_rRectF1.top,m_rRectF1.right,m_rRectF1.bottom);
		 ...
		 canvas.drawArc(rect, 
			...
			for(int i=0;i<count2;i++){						 
				 ...
				 canvas.drawLine(0, y_p, 0, y_p-6, m_oPaint);	
			}
			canvas.restore();
			...
			Path path = new Path();
			path.addArc(new RectF(-x_p, -y_p, x_p, y_p), 95, -95);		
			canvas.rotate(-360/scale);
			for(int i=0;i<scale;i++){						 
				canvas.rotate(360/scale);
				 if(i%2==0){
					 float label_value = maxValue/scale *(float)i;
					 DecimalFormat decimalFloat = new DecimalFormat("0"); 
					 String str = decimalFloat.format(label_value);
					 canvas.drawTextOnPath(str, path, 0, -14, m_oPaint);
				 }
	...
	public boolean updateValue()
	{
		...
		if(strValue.equals(oldSignalValue)){
        	...
        	oldSignalValue = strValue;
     	 	parseFontcolor(oRealTimeData.strData);   
