...
public class Dial_C extends View implements IObject, ViewObjectSetCallBack {
	...
	protected void onDraw(Canvas canvas) {
		...
		float angle = 0;
		if (((data_value - minValue) - (maxValue - minValue)) >= 0) {
			angle = 270;
		} else {
	...
	private ViewObjectBase base = new Dial_CObject();
	...
	public void onCall() {
		...
		((Dial_CObject) base).setBackgroundColor(ViewObjectColorUtil.getColor(backgroundColor));
		((Dial_CObject) base).setBorderColor(ViewObjectColorUtil.getColor(borderColor));
		((Dial_CObject) base).setFillColor(ViewObjectColorUtil.getColor(fillColor));
		((Dial_CObject) base).setLineColor(ViewObjectColorUtil.getColor(lineColor));
		((Dial_CObject) base).setWarmPerColor(ViewObjectColorUtil.getColor(warmPerColor));

		((Dial_CObject) base).setMaxValue(maxValue);
		((Dial_CObject) base).setMinValue(minValue);
		((Dial_CObject) base).setWarmPer(warnPer);
		((Dial_CObject) base).setScale(scale);
	}
