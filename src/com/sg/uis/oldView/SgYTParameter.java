...
public class SgYTParameter extends TextView implements IObject, ViewObjectSetCallBack {
	public SgYTParameter(Context context) {
		...
		m_oEditText.setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_CANCEL:
					/*
					 * //imm.showSoftInput(m_oEditText, 0); m_oEditText.setFocusable(true);
					 ...
					 */
					/*
					 * boolean isOpen = imm.isActive(); if (!isOpen) { imm.toggleSoftInput(0,
					 * InputMethodManager.HIDE_NOT_ALWAYS); // 隐藏任何可能出现的软键盘 }
					 */
		 ...
		 * 
		 * @Override public boolean onLongClick(View v) { imm.showSoftInput(m_oEditText,
		 * InputMethodManager.SHOW_FORCED);
		 * m_oEditText.setSelection(m_oEditText.getText().length()); //光标移动最后
		 * 
		 * bEditable = true; SgYTParameter.this.setEnabled(bEditable); return true; }
		 * });
		 */
	...
	public void doLayout(boolean bool, int l, int t, int r, int b) {
		...
		if (m_rRenderWindow.isLayoutVisible(m_rBBox)) {
			...
			m_oEditText.layout(nX, nY + nHeight / 10, nX + (int) (nWidth * 0.7f),
					nY + (int) (nHeight * 1.0f) - nHeight / 10);
		}
	}
	...
	private void onClicked() {
		...
		try {
			...
			// new AlertDialog.Builder(this.getContext()).setTitle("错误")
			// .setMessage("请输入合法数值！") .show();
			m_oEditText.setText("");
		...
		if ("".equals(m_strCmdExpression) == false) {
			synchronized (m_rRenderWindow.m_oShareObject) {
				...
				m_rRenderWindow.m_oShareObject.m_mapCmdCommand.put(getUniqueID(),
						m_oEditText.getText().toString().trim());
			}
		}
	}
	 ...
	 *             public BitmapDrawable getImageDrawable(String path) throws
	 *             IOException { //打开文件 File file = new File(path);
	 *             if(!file.exists()) { return null; }
	 * 
	 *             ByteArrayOutputStream outStream = new ByteArrayOutputStream();
	 *             byte[] bt = new byte[BUFFER_SIZE];
	 * 
	 *             //得到文件的输入流 InputStream in = new FileInputStream(file);
	 * 
	 *             //将文件读出到输出流中 int readLength = in.read(bt); while (readLength !=
	 *             -1) { outStream.write(bt, 0, readLength); readLength =
	 *             in.read(bt); }
	 * 
	 *             in.close();
	 * 
	 *             //转换成byte 后 再格式化成位图 byte[] data = outStream.toByteArray(); Bitmap
	 *             bitmap = BitmapFactory.decodeByteArray(data, 0, data.length);//
	 *             生成位图 BitmapDrawable bd = new BitmapDrawable(bitmap);
	 * 
	 *             return bd; }
	 */
	...
	public void onCall() {
		...
		if (!m_strCmdExpression.equals("")) {
			((YTParameterObject) base).setClick(true);
		} else {
			((YTParameterObject) base).setClick(false);
		}
	...
	public void onControl(Object obj) {
		...
		if (!str.equals("")) {
			synchronized (m_rRenderWindow.m_oShareObject) {
				m_rRenderWindow.m_oShareObject.m_mapCmdCommand.put(getUniqueID(), (String) obj);
			}
		}
